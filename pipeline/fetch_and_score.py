...
Supabase sync-data Edge Function.
...
UNIVERSE: list[str] = [
    ...
    "LMT", "CIEN", "FIX", "MPC", "MU", "AMAT", "NVDA", "META", "TSLA",
    "AMZN", "MSFT", "AAPL", "GE", "FDX", "GOOGL",
]
...
def score_ticker(ticker: str, df: pd.DataFrame) -> dict:
    ...
    Returns a dict matching the stocks table schema / sync-data payload contract.
    """
    close = df["Close"]
    high  = df["High"]
    low   = df["Low"]
    ...
    distance_52w = round((price - yr_low) / yr_range * 100 - 50, 1) if yr_range > 0 else 0.0
